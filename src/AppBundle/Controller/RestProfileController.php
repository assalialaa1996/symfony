...
class RestProfileController extends FOSRestController implements ClassResourceInterface
{
    ...
    private function updateProfile(Request $request, $clearMissing = true, UserInterface $user)
    {
        ...
        //Edit customer stripe information
        $compte=$this->getDoctrine()->getRepository('EntrepriseBundle:Compte')->findOneByuser($user);
        $cutomer_id=$compte->getStripeId();


        \Stripe\Stripe::setApiKey("sk_test_R6f3tuAsgbkH45pSm6tcr4s7");
        $cu = \Stripe\Customer::retrieve("$cutomer_id");
        $cu->description = $user->getUsername();
